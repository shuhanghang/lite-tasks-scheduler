...
def get_all_available_jobs():
    ...
    for job_class_package in settings.JOB_CLASS_PACKAGES:
        try:
            ...
            logger.warn('Cannot import %s. Ignore it for now.' %
                        job_class_package)
            continue
                ...
                for property in dir(job_module):
                    ...
                    try:
                        if issubclass(module_property, job.JobBase):
                            job_class_result.append(
                                module_property.meta_info())
                    except TypeError:
                        pass

    for job_config_class_package in settings.JOB_CONFIG_PACKAGES:
        try:
            package = importlib.import_module(job_config_class_package)
        except ImportError:
            logger.warn('Cannot import %s. Ignore it for now.' %
                        job_config_class_package)
            continue
        ...
        for dir_path in package.__path__:
            ...
            for file in files:
                ...
                if filename == '__init__.py':
                    continue
                module_name = filename[:-3]
                job_module = '%s.%s' % (job_config_class_package, module_name)
                job_config_result.append(
                    {'job_configs_class_string': job_module})

    return job_class_result, job_config_result
