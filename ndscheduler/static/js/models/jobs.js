...
define(['utils',
  ...
  return Backbone.Collection.extend({
    ...
    /**
         * Configure a job.
         */
    configureJob: function(data) {
      $.ajax({
        url: config.config_url,
        type: 'PUT',
        data: JSON.stringify(data),
        contentType: 'application/json; charset=utf-8',
        success: this._configureJobSuccess,
        error: this._configureJobError
      });
    },    

    _configureJobSuccess: function(e) {
      utils.alertSuccess(e['msg']);
    },

    _configureJobError: function() {
      utils.alertError("Failed to configure the job.");
    },

    /**
         * Get Config of job.
         */
    getConfig: function(configBindString) {
      return $.ajax({
        url: config.config_url,
        type: 'GET',
        data: {'config_bind': configBindString},
        // contentType: 'application/json; charset=utf-8',
        error: this._getJobConfigError
      })
    },

    _getJobConfigError: function() {
      utils.alertError('Failed to get config.');
    },
